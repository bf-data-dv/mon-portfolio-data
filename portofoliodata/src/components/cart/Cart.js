...
const Cart = ({ cart, onRemoveItem, onUpdateQuantity, onUpdateOptions, onBack, onClearCart, session }) => { // 👈 On ajoute session ici
  ...
  /**
   * ANNOTATION : Cette fonction appelle l'Edge Function "upload-to-s3"
   * pour transmettre les données de la vente vers ton bucket AWS S3.
   */
const sendToS3Lambda = async (venteData) => {
    try {
      // 1. On récupère la date actuelle
      const now = new Date();
      const annee = now.getFullYear();
      const mois = String(now.getMonth() + 1).padStart(2, '0'); // Ajoute un zéro si besoin
      const jour = String(now.getDate()).padStart(2, '0');
      
      // 2. On crée le chemin structuré dynamiquement
      const path = `raw/ventes/annee=${annee}/mois=${mois}/jour=${jour}/vente_${Date.now()}.json`;
      
      await supabase.functions.invoke('upload-to-s3', {
        body: JSON.stringify({ data: venteData, path: path }),
        headers: { 'Content-Type': 'application/json' }
      });

      console.log("Succès : Fichier archivé dans", path);
    } catch (err) {
      console.warn("Erreur mineure S3 :", err);
    }
  };
  /**
   * handleCheckout : Processus de validation.
   * 1. Vérification auth.
   * 2. Boucle sur les articles.
   * 3. Mise à jour stock Supabase.
   * 4. Insertion historique vente.
   * 5. Appel S3 pour archivage.
   */
      ...
      for (const item of cart) {
        ...
        if (item.finish) {
          ...
          if (qualityData) {
            ...
            if (lowerFinish === 'excellence') targetQualityId = 'ce4aab43-c751-42cf-9c4d-63cfb7b775ce';
            else if (lowerFinish === 'prestige') targetQualityId = '3539c329-ce38-4e38-b029-c584faeb0244';
            else if (lowerFinish === 'confort') targetQualityId = 'c6b982bc-78e2-4ec7-84a0-8bec822b93e3';
            else if (lowerFinish === 'classique +') targetQualityId = '386b8f69-39ad-483d-9c9f-9915f861b1c6';
          }
        }

        if (!targetQualityId) throw new Error(`Impossible de lier la finition "${item.finish}" à un identifiant unique.`);
        ...
        const updatedFields = { tapis_avt_stock: Math.max(0, (currentStock.tapis_avt_stock || 0) - item.quantity) };
        ...
        if (item.options?.trunk) updatedFields.tapis_coffre_stock = Math.max(0, (currentStock.tapis_coffre_stock || 0) - item.quantity);

        await supabase.from('carpet_stock').update(updatedFields).eq('inventory_id', realId);

        const itemsVendus = ['avant'];
        if (item.options?.rear) itemsVendus.push('arriere');
        if (item.options?.trunk) itemsVendus.push('coffre');

        const { error: salesError, data: newSale } = await supabase
          .from('sales_history')
          .insert([{
            user_id: userId, inventory_id: realId, marque: (item.brand || 'INCONNUE').toUpperCase(),
            modele_voiture: item.modelName || 'Inconnu', tapis_vendu: itemsVendus.join(', '),
            items_details: itemsVendus, quantite: item.quantity, total_price: item.price * item.quantity,
        ...
        // ANNOTATION : Envoi des détails vers le stockage externe S3
        await sendToS3Lambda({ ...item, saleId: newSale.id, userId: userId });
      }
    ...
    <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20 px-4">
      ...
      <div className="flex items-center gap-6 mb-10">
        ...
        <div className="text-left">
          <h2 className="text-2xl md:text-3xl font-black text-white uppercase tracking-tight italic leading-none">Mon Panier</h2>
      ...
      {cart.length === 0 ? (
        <div className="bg-slate-900/50 border border-white/5 rounded-[40px] py-20 text-center backdrop-blur-xl">
          ...
          <button onClick={onBack} className="mt-6 text-indigo-400 font-bold uppercase text-xs hover:underline">Retourner au catalogue</button>
        </div>
      ) : (
              ...
              <div key={`${item.id}-${index}`} className="bg-white rounded-[30px] p-6 flex flex-col md:flex-row items-center justify-between gap-6 shadow-xl border border-white">
                ...
                <div className="flex items-center gap-5 w-full">
                  ...
                  <div className="text-left flex-grow">
                    <p className="text-[10px] font-black text-indigo-600 uppercase tracking-widest leading-none mb-1">{item.brand} {item.modelName}</p>
                    <h3 className="text-lg font-black text-slate-900 uppercase italic leading-none mb-3">Finition {item.finish}</h3>
                    <div className="space-y-1 border-l-2 border-slate-100 pl-4">
                      ...
                      {item.options?.rear && (
                        <div className="flex justify-between w-full md:w-64 items-center">
                          <button onClick={() => onUpdateOptions(index, 'rear')} className="p-0.5 bg-red-50 text-red-500 rounded hover:bg-red-500 hover:text-white"><X size={10}/></button>
                          <span className="text-slate-500 text-[10px] uppercase font-bold">Option Arrière + Pont</span>
                          <span className="text-indigo-600 text-[10px] font-black">+{Number(item.optionPrices?.rear || 0).toFixed(2)}€</span>
                        </div>
                      )}
                      {item.options?.trunk && (
                        <div className="flex justify-between w-full md:w-64 items-center">
                          <button onClick={() => onUpdateOptions(index, 'trunk')} className="p-0.5 bg-red-50 text-red-500 rounded hover:bg-red-500 hover:text-white"><X size={10}/></button>
                          <span className="text-slate-500 text-[10px] uppercase font-bold">Option Coffre</span>
                          <span className="text-indigo-600 text-[10px] font-black">+{Number(item.optionPrices?.trunk || 0).toFixed(2)}€</span>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-6 w-full md:w-auto justify-between md:justify-end">
                  ...
                  <div className="flex items-center bg-slate-100 rounded-2xl p-1.5">
                    <button onClick={() => onUpdateQuantity(index, -1)} className="p-1.5 text-slate-600 hover:text-indigo-600"><Minus size={16} /></button>
                    <span className="w-10 text-center font-black text-slate-900">{item.quantity}</span>
                    <button onClick={() => onUpdateQuantity(index, 1)} className="p-1.5 text-slate-600 hover:text-indigo-600"><Plus size={16} /></button>
                  </div>
                  <button onClick={() => onRemoveItem(index)} className="p-3 bg-red-50 text-red-500 hover:bg-red-500 hover:text-white rounded-2xl"><Trash2 size={20} /></button>
                </div>
              </div>
            ))}
          </div>
          <div className="lg:col-span-1">
            <div className="bg-indigo-600 rounded-[40px] p-8 sticky top-8 shadow-2xl">
              ...
              <div className="space-y-4 mb-10">
                <div className="flex justify-between text-indigo-200 font-bold uppercase text-[11px] tracking-widest">
                  <span>Sous-total</span><span className="text-white">{totalPrice.toFixed(2)}€</span>
                </div>
                <div className="h-px bg-white/10 w-full" />
                <div className="flex justify-between items-end">
                  <span className="text-white font-black uppercase text-xs tracking-[0.2em]">Total</span>
                  <span className="text-white font-black text-3xl italic leading-none">{totalPrice.toFixed(2)}€</span>
                </div>
              </div>
              <button onClick={handleCheckout} disabled={isSubmitting} className="w-full bg-emerald-500 text-white font-black uppercase py-6 rounded-[25px] transition-all flex items-center justify-center gap-3">
                {isSubmitting ? <><Loader2 className="animate-spin" /> Traitement...</> : "Valider la commande"}
              </button>
            </div>
          </div>
        </div>
